...
// Backend API URL (relative path - served from same origin)
const API_URL = '/api';
...
function setupInputs() {
    ...
    // CAD Upload - Server-side parsing
    document.getElementById('cad-upload').addEventListener('change', async (e) => {
        ...
        try {
            // Send to backend for server-side parsing with Ab4d.OpenCascade
            const formData = new FormData();
            formData.append('file', file);

            const response = await fetch(`${API_URL}/import/cad`, {
                method: 'POST',
                body: formData
            });
            ...
            if (result.success && result.meshes && result.meshes.length > 0) {
                ...
                showImportStatus(`Loaded: ${file.name} (${result.partCount} parts)`);
            } else {
                const errorMsg = result.error || 'Could not parse CAD file';
                showImportStatus(errorMsg, true);
            }
        } catch (err) {
            console.error('CAD import error:', err);
            showImportStatus('Error loading CAD file. Check server connection.', true);
        }
